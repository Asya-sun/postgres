...
_PG_init(void)
{
	DefineCustomBoolVariable("test_mes.enabled",
...
hello_start_worker(void)
{
	...
	snprintf(worker_name, sizeof(worker_name), "%s%d", "test_mes healthcheck", MyProcPid);

	MemSet(&worker, 0, sizeof(BackgroundWorker));
	worker.bgw_flags = BGWORKER_SHMEM_ACCESS;
	worker.bgw_start_time = BgWorkerStart_PostmasterStart;
	strcpy(worker.bgw_library_name, "test_mes");
	strcpy(worker.bgw_function_name, "hello_main");
	strcpy(worker.bgw_name, worker_name);
	strcpy(worker.bgw_type, "test_mes healthcheck");
...
hello_main(Datum main_arg)
{
	...
	flag = 0;
	for (MonitorEvent event = ME_B; event < MONITOR_EVENT_NUM_TYPES; event++) {
		if (flag == 0) {
	...
	while (true)
	{
		...
		if (!flag) {
			...
			if (subscribed) {
				flag = UnsubscribeFromMonitorEvent(ME_C, socket_fd, MyProcPid);
				if (flag == 1) {
					elog(LOG, "[%d] DIDN'T SUCCEED unsibscribing from event %d", MyProcPid, ME_C);
				} else {
					subscribed = false;
					elog(LOG, "[%d] SUCCEDED unsibscribing from event %d", MyProcPid, ME_C);
				}
			}
